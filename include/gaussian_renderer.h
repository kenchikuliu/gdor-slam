...
public:
    ...
    // Dense metric depth and accumulated opacity rendering.
    static std::tuple<torch::Tensor, torch::Tensor> renderDepthWithOpacity(
        std::shared_ptr<GaussianKeyframe> viewpoint_camera,
        int image_height,
        int image_width,
        std::shared_ptr<GaussianModel> gaussians,
    ...
    // Backward-compatible wrapper returning normalized metric depth.
    static torch::Tensor renderDepth(
        std::shared_ptr<GaussianKeyframe> viewpoint_camera,
        int image_height,
        int image_width,
        std::shared_ptr<GaussianModel> gaussians,
