...
public:
    ...
    // DyGeoFusion-SLAM+: Static weight for soft-weight 3DGS optimization
    // static_weight_ = 1 - P_dyn, where P_dyn is the dynamic probability
    // Higher values mean more static (more reliable for 3DGS training)
    torch::Tensor static_weight_;  ///< [1, H, W] tensor, values in [0, 1]
    bool has_static_weight_ = false;
