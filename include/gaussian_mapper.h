...
struct GaussianMappingAuditCounters
{
    std::uint64_t keyframes_with_static_weight = 0;
    std::uint64_t mapping_weight_pixels = 0;
    std::uint64_t mapping_static_pixels = 0;
    std::uint64_t orb_map_points_considered = 0;
    std::uint64_t orb_map_points_rejected_static_mask = 0;
    std::uint64_t rgbd_densification_candidates = 0;
    std::uint64_t rgbd_densification_rejected_static_mask = 0;
};
...
public:
    ...
    /**
     * @brief Render depth map from 3DGS model at given camera pose
     *
     * Uses opacity-normalized differentiable Gaussian rasterization.
     *
     * @param Tcw Camera pose (world to camera transform)
     * @param width Output image width
     * @param height Output image height
     * @param fx Camera focal length x (pixels)
     * @param fy Camera focal length y (pixels)
     * @param cx Camera principal point x (pixels)
     * @param cy Camera principal point y (pixels)
     * @param min_opacity Minimum accumulated render opacity for valid depth
     * @return cv::Mat Depth map in meters (CV_32FC1), empty if not ready
     */
    cv::Mat renderDepthFromPose(
        const Sophus::SE3f &Tcw,
        const int width,
        const int height,
        const float fx,
        ...
        const float cx,
        const float cy,
        const float min_opacity = 0.0f);

    int getIteration();
    void increaseIteration(const int inc = 1);
    GaussianMappingAuditCounters getMappingAuditCounters() const;
    ...
    void setLambdaDepth(const float lambda_depth);
    ...
    std::shared_ptr<GaussianModel> getGaussianModel() const { return gaussians_; }
...
protected:
    ...
    // Seeded generator used for reproducible keyframe sampling.
    std::mt19937 rng_;

    std::atomic<std::uint64_t> keyframes_with_static_weight_{0};
    std::atomic<std::uint64_t> mapping_weight_pixels_{0};
    std::atomic<std::uint64_t> mapping_static_pixels_{0};
    std::atomic<std::uint64_t> orb_map_points_considered_{0};
    std::atomic<std::uint64_t>
        orb_map_points_rejected_static_mask_{0};
    std::atomic<std::uint64_t> rgbd_densification_candidates_{0};
    std::atomic<std::uint64_t>
        rgbd_densification_rejected_static_mask_{0};
