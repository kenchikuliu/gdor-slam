...
void YOLOv8::postprocess(std::vector<Object>& objs)
{
    ...
    // Check if this is end2end format (4 outputs) or standard format (1 output)
    if (this->num_outputs == 1) {
        // Standard YOLO output: use postprocess_standard
        postprocess_standard(objs, 0.5f, 0.45f);
        return;
    }

    // End2end format (4 outputs)
...
void YOLOv8::postprocess_standard(std::vector<Object>& objs, float conf_thresh, float nms_thresh)
{
    objs.clear();

    // Standard YOLO output shape: [1, 84, 8400] where 84 = 4 (bbox) + 80 (classes)
    // Transposed to [1, 8400, 84] for easier processing
    auto* output = static_cast<float*>(this->host_ptrs[0]);
    auto& binding = this->output_bindings[0];

    int num_channels = binding.dims.d[1];  // 84 (4 + 80 classes)
    int num_anchors = binding.dims.d[2];   // 8400
    ...
    for (int i = 0; i < num_anchors; i++) {
        ...
        float x0 = cx - bw / 2.0f - dw;
        float y0 = cy - bh / 2.0f - dh;
        float x1 = cx + bw / 2.0f - dw;
        float y1 = cy + bh / 2.0f - dh;
    ...
    for (size_t i = 0; i < proposals.size(); i++) {
        ...
        objs.push_back(proposals[i]);

        for (size_t j = i + 1; j < proposals.size(); j++) {
            if (suppressed[j]) continue;

            // Calculate IoU
            float x0 = std::max(proposals[i].rect.x, proposals[j].rect.x);
            float y0 = std::max(proposals[i].rect.y, proposals[j].rect.y);
            float x1 = std::min(proposals[i].rect.x + proposals[i].rect.width,
                               proposals[j].rect.x + proposals[j].rect.width);
            float y1 = std::min(proposals[i].rect.y + proposals[i].rect.height,
                               proposals[j].rect.y + proposals[j].rect.height);

            float inter_w = std::max(0.0f, x1 - x0);
            float inter_h = std::max(0.0f, y1 - y0);
            float inter_area = inter_w * inter_h;

            float area_i = proposals[i].rect.width * proposals[i].rect.height;
            float area_j = proposals[j].rect.width * proposals[j].rect.height;
