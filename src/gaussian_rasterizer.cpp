...

{
    ...
    if (!has_shs)
        shs = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    if (!has_colors_precomp)
        colors_precomp = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    if (!has_scales)
        scales = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    if (!has_rotations)
        rotations = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
    if (!has_cov3D_precomp)
        cov3D_precomp = torch::empty({0}, torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA));
