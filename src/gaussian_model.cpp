...
{
    auto features_dc = this->features_dc_;
    auto features_rest = this->features_rest_;

    // Defensive check for tensor size mismatch (can occur during concurrent updates)
    if (features_dc.size(0) != features_rest.size(0)) {
        std::cerr << "[WARNING] GaussianModel::getFeatures() - Size mismatch detected: "
                  << "features_dc_.size(0)=" << features_dc.size(0)
                  << " vs features_rest_.size(0)=" << features_rest.size(0)
                  << ". Trimming to minimum size." << std::endl;

        auto min_size = std::min(features_dc.size(0), features_rest.size(0));
        features_dc = features_dc.index({torch::indexing::Slice(0, min_size)});
        features_rest = features_rest.index({torch::indexing::Slice(0, min_size)});
    }

    return torch::cat({features_dc.clone(), features_rest.clone()}, /*dim=*/1);
}
