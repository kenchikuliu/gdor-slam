...
void GaussianMapper::run()
{
        ...
        if (hasMetInitialMappingConditions()) {
            ...
            std::vector<ORB_SLAM3::MapPoint*> vpMPs;
            {
                ...
                for (const auto& pMP : vpMPs){
                    orb_map_points_considered_.fetch_add(
                        1, std::memory_order_relaxed);
                    if (!ORB_SLAM3::mapPointPassesGaussianStaticMask(pMP)) {
                        orb_map_points_rejected_static_mask_.fetch_add(
                            1, std::memory_order_relaxed);
                        continue;
                    }
                    Point3D point3D;
    ...
    // A bounded smoke run can stop before the Gaussian initialization criteria
    // are met. Do not enter training or serialization with an uninitialized model.
    if (!initial_mapped_) {
        signalStop();
        return;
    }
...
void GaussianMapper::trainForOneIteration()
{
    ...
    // Loss computation - DyGeoFusion-SLAM+: Use weighted loss if static_weight is available
    float lambda_dssim = lambdaDssim();
    float lambda_depth = lambdaDepth();  // NEW: depth supervision weight
    torch::Tensor loss;
    torch::Tensor Ll1;
    torch::Tensor L_depth;  // NEW: depth loss term
    torch::Tensor training_static_weight;

    if (viewpoint_cam->has_static_weight_) {
        // Get static weight and resize if using gaussian pyramid
        torch::Tensor static_weight = viewpoint_cam->static_weight_;

        // Resize weight to match current training resolution if needed
        if (static_weight.size(1) != image_height || static_weight.size(2) != image_width) {
            // Use bilinear interpolation for smooth weight scaling
            static_weight = torch::nn::functional::interpolate(
                static_weight.unsqueeze(0),  // [1, 1, H, W]
        ...
        // Apply undistortion mask to weight
        static_weight = static_weight * mask;
        training_static_weight = static_weight;

        // Use weighted loss functions
        Ll1 = loss_utils::weighted_l1_loss(masked_image, gt_image, static_weight);
        auto weighted_dssim = loss_utils::weighted_ssim(masked_image, gt_image, static_weight, device_type_);
        loss = (1.0 - lambda_dssim) * Ll1 + lambda_dssim * (1.0 - weighted_dssim);
    } else {
    ...
    // NEW: Add depth supervision loss for RGB-D sensor
    if (lambda_depth > 0.0f && sensor_type_ == RGBD && !viewpoint_cam->img_auxiliary_undist_.empty()) {
        // Render depth from 3DGS model
        auto depth_render = GaussianRenderer::renderDepthWithOpacity(
            viewpoint_cam,
            image_height,
            image_width,
            gaussians_
        );
        torch::Tensor rendered_depth = std::get<0>(depth_render);
        torch::Tensor rendered_opacity = std::get<1>(depth_render);

        // Convert sensor depth (cv::Mat) to torch::Tensor
        torch::Tensor sensor_depth = tensor_utils::cvMat2TorchTensor_Float32(
            viewpoint_cam->img_auxiliary_undist_, device_type_);

        // Resize sensor depth if using gaussian pyramid
        // sensor_depth is [H, W] for single-channel depth image
        if (sensor_depth.size(0) != image_height || sensor_depth.size(1) != image_width) {
            sensor_depth = torch::nn::functional::interpolate(
                sensor_depth.unsqueeze(0).unsqueeze(0),  // [H,W] -> [1, 1, H, W]
                torch::nn::functional::InterpolateFuncOptions()
                    .size(std::vector<int64_t>{image_height, image_width})
                    .mode(torch::kBilinear)
                    .align_corners(false)
        ...
        // Create valid depth mask: depth > min_depth and depth < max_depth
        torch::Tensor valid_mask = (sensor_depth > RGBD_min_depth_) & (sensor_depth < RGBD_max_depth_);
        valid_mask = valid_mask & (rendered_opacity > 0.01f);
        valid_mask = valid_mask.to(torch::kFloat32);

        // Apply undistortion mask
        valid_mask = valid_mask * mask;
        if (training_static_weight.defined()) {
            valid_mask = valid_mask * training_static_weight.squeeze(0);
        }

        // Compute depth loss (L1) only on valid pixels
        const float valid_weight = valid_mask.sum().item<float>();
        if (valid_weight > 1e-6f) {
            torch::Tensor depth_diff = torch::abs(rendered_depth - sensor_depth);
            L_depth = (depth_diff * valid_mask).sum() / valid_weight;

            // Add depth loss to total loss
...
void GaussianMapper::combineMappingOperations()
{
    ...
    while (pSLAM_->getAtlas()->hasMappingOperation()) {
        ...
        orb_map_points_considered_.fetch_add(
            opr.mapPointsConsidered(), std::memory_order_relaxed);
        orb_map_points_rejected_static_mask_.fetch_add(
            opr.mapPointsRejectedStaticMask(),
...
void GaussianMapper::bindStaticWeight(
    std::shared_ptr<GaussianKeyframe> keyframe,
    const cv::Mat& static_weight,
    camera_id_t camera_id)
{
    if (static_weight.empty()) {
        keyframe->has_static_weight_ = false;
        return;
    }
    ...
    try {
        ...
        if (sensor_type_ == STEREO) {
            undistorted = static_weight;
        } else {
            camera.undistortImage(static_weight, undistorted);
        }
    } catch (std::out_of_range) {
        undistorted = static_weight;
    }
    ...
    cv::Mat weight_float;
    if (undistorted.type() != CV_32FC1) {
        undistorted.convertTo(weight_float, CV_32FC1);
    } else {
        weight_float = undistorted;
    }
    auto weight_tensor = torch::from_blob(
        weight_float.data,
        {weight_float.rows, weight_float.cols},
        torch::kFloat32).clone();
    keyframe->static_weight_ =
        weight_tensor.unsqueeze(0).to(device_type_);
    keyframe->has_static_weight_ = true;

    cv::Mat static_pixels;
    cv::compare(
        weight_float, 0.5f, static_pixels, cv::CMP_GE);
    keyframes_with_static_weight_.fetch_add(
        1, std::memory_order_relaxed);
    mapping_weight_pixels_.fetch_add(
        static_cast<std::uint64_t>(weight_float.total()),
        std::memory_order_relaxed);
    mapping_static_pixels_.fetch_add(
        static_cast<std::uint64_t>(
            cv::countNonZero(static_pixels)),
        std::memory_order_relaxed);
}
...
void GaussianMapper::handleNewKeyframe(
    std::tuple< unsigned long/*Id*/,
                ...
                std::vector<float>,
                std::string,
                cv::Mat/*staticWeight (DyGeoFusion-SLAM+)*/> &kf)
{
    ...
    // Bind mapping confidence before any RGB-D point initialization.
    bindStaticWeight(pkf, std::get<9>(kf), std::get<1>(kf));
...
// std::shuffle(kfid_shuffle_.begin(), kfid_shuffle_.end(), g);
    ...
    std::iota(kfid_shuffle_.begin(), kfid_shuffle_.end(), 0);
    std::shuffle(kfid_shuffle_.begin(), kfid_shuffle_.end(), rng_);
...
GaussianMapper::useOneRandomKeyframe()
{
    ...
    std::uniform_int_distribution<int> distribution(0, nkfs - 1);
    int random_cam_idx = distribution(rng_);
    auto random_cam_it = scene_->keyframes().begin();
    for (int cam_idx = 0; cam_idx < random_cam_idx; ++cam_idx)
        ++random_cam_it;
...
// savePly(result_dir_ / (std::to_string(getIteration()) + "_" + std::to_string(pkf->fid_) + "_0_before_inactive_geo_densify"));
        ...
        const std::uint64_t candidates =
            static_cast<std::uint64_t>(
                point_valid_flags.sum().item<std::int64_t>());
        if (pkf->has_static_weight_) {
            auto static_weight =
                pkf->static_weight_.squeeze(0).flatten(0, 1);
            point_valid_flags = torch::logical_and(
                point_valid_flags,
                static_weight >= 0.5f);
        }
        const std::uint64_t accepted =
            static_cast<std::uint64_t>(
                point_valid_flags.sum().item<std::int64_t>());
        rgbd_densification_candidates_.fetch_add(
            candidates, std::memory_order_relaxed);
        rgbd_densification_rejected_static_mask_.fetch_add(
            candidates - accepted, std::memory_order_relaxed);
...
cv::Mat GaussianMapper::renderDepthFromPose(
    const Sophus::SE3f &Tcw,
    const int width,
    const int height,
    const float fx,
    ...
    const float min_opacity)
{
    ...
    // Create temporary GaussianKeyframe for dense rendering
    std::shared_ptr<GaussianKeyframe> temp_kf = std::make_shared<GaussianKeyframe>(0);
    {
        std::unique_lock<std::mutex> lock_render(mutex_render_);

        // GaussianKeyframe::setPose expects the world-to-camera transform.
        temp_kf->setPose(
            Tcw.unit_quaternion().cast<double>(),
            Tcw.translation().cast<double>());

        // Set camera intrinsics
        temp_kf->camera_id_ = 0;
        temp_kf->camera_model_id_ = 1;  // Pinhole model
        temp_kf->image_height_ = height;
        temp_kf->image_width_ = width;

        // Set intrinsic parameters [fx, fy, cx, cy]
        temp_kf->intr_.resize(4);
        temp_kf->intr_[0] = fx;
        temp_kf->intr_[1] = fy;
        temp_kf->intr_[2] = cx;
        temp_kf->intr_[3] = cy;
        ...
        temp_kf->FoVx_ = graphics_utils::focal2fov(fx, width);
        temp_kf->FoVy_ = graphics_utils::focal2fov(fy, height);
        temp_kf->set_camera_ = true;

        // Set depth range
        temp_kf->znear_ = z_near_;
        temp_kf->zfar_ = z_far_;

        // CRITICAL: Compute transform tensors (world_view_transform_, camera_center_, etc.)
        temp_kf->computeTransformTensors();
    }

    // Call dense 3DGS depth rendering
    ...
    torch::Tensor opacity_tensor;
    {
        ...
        depth_tensor = std::get<0>(depth_render);
        opacity_tensor = std::get<1>(depth_render);
    }
    ...
    if (min_opacity > 0.0f) {
        depth_tensor = torch::where(
            opacity_tensor >= min_opacity,
            depth_tensor,
            torch::zeros_like(depth_tensor));
    }

    // Move to CPU and ensure contiguous memory
    depth_tensor = depth_tensor.to(torch::kCPU).contiguous();

    // Create cv::Mat wrapper (shares memory, no copy)
    cv::Mat depth_map(height, width, CV_32FC1, depth_tensor.data_ptr<float>());

    // Clone to create independent copy (tensor will be destroyed)
...
GaussianMapper::getMappingAuditCounters() const
{
    GaussianMappingAuditCounters counters;
    counters.keyframes_with_static_weight =
        keyframes_with_static_weight_.load(
            std::memory_order_relaxed);
    counters.mapping_weight_pixels =
        mapping_weight_pixels_.load(std::memory_order_relaxed);
    counters.mapping_static_pixels =
        mapping_static_pixels_.load(std::memory_order_relaxed);
    counters.orb_map_points_considered =
        orb_map_points_considered_.load(
            std::memory_order_relaxed);
    counters.orb_map_points_rejected_static_mask =
        orb_map_points_rejected_static_mask_.load(
            std::memory_order_relaxed);
    counters.rgbd_densification_candidates =
        rgbd_densification_candidates_.load(
            std::memory_order_relaxed);
    counters.rgbd_densification_rejected_static_mask =
        rgbd_densification_rejected_static_mask_.load(
            std::memory_order_relaxed);
    return counters;
}
...
void GaussianMapper::setLambdaDepth(const float lambda_depth)
{
    std::unique_lock<std::mutex> lock(mutex_settings_);
    opt_params_.lambda_depth_ = lambda_depth;
}
