...
The route invariant is frame-local and admission-local: a frame that contains recovered tracking support does not receive a persistent-map admission privilege.
Every ORB MapPoint, Gaussian initialization, RGB-D densification candidate, and Gaussian loss term is still admitted or weighted only through $W_t^{\mathrm{map}}$; recovered support cannot set that weight to one, bypass keyframe static-weight checks, or waive the conservative mapping boundary.
