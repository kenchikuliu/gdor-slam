...
\begin{table*}[t]
\centering
\caption{Earlier same-source tracking evidence. Values are three-seed ATE RMSE in centimeters; lower is better. Every reported mean improvement is accompanied by its failure case, median, and win/tie/loss count.}
...
\begin{table*}[t]
\centering
\caption{Safety, efficiency, and DYN-19 mapping diagnostics. Panel A reports the DYN-15 All7 mean. Panel B represents all 72 mapping cells: 12 sequence/seed cells per method in each pose mode. Completeness and ghost-risk columns are occluded-background proxies, not independent ground truth.}
...
Published DGS-SLAM, DG-SLAM, DyPho-SLAM, and DynaSLAM values remain related-work context rather than protocol-matched DYN-19 rows.
...
Both therefore remain outside the main table until online-pose, association, mask-provenance, artifact, and no-failure eligibility gates pass.
