...
\begin{figure*}[t]
  ...
  \caption{GDOR-SLAM separates support qualification, transient tracking support, and persistent mapping weight. The MapMatched control uses the same persistent weight as Full but disables temporal recovery and adaptive tracking reuse.}
  \label{fig:method}
\end{figure*}
...
The Full configuration uses temporal-depth recovery, residual-flow protection, risk conditioning, and adaptive feature replenishment.
DYN-19 also evaluates nested T+M, T+F+M, and T+F+R+M operating points.
These rows form an ordered configuration ladder: they expose whether additional guards help the complete route, but do not identify independent factor effects.
...
\text{Recovered+M} & \text{Eq.~\eqref{eq:tracking-support}} & 1-\bar D_t \\
\text{Full-NoM} & \text{Eq.~\eqref{eq:tracking-support}} & S_t^{\mathrm{track}}
\end{array}
...
The Full--MapMatched comparison therefore tests qualified tracking reuse on top of the same persistent mapping weight; it is not a comparison between two Gaussian-admission masks.

\subsection{Implementation Details}

The frozen Full configuration applies semantic inference on every frame after a 50-frame system warmup and starts temporal background refinement after five frames.
...
These values were frozen in the DYN-19 plan and were not adjusted using its outcomes.
