...
Eigen::Matrix<double, 6, 6> Optimizer::PosePriorInformationForG2O(
    const Eigen::Matrix<float, 6, 6>& sophusInformation)
{
    static constexpr int kG2oToSophus[6] = {3, 4, 5, 0, 1, 2};
    Eigen::Matrix<double, 6, 6> reordered;
    for (int row = 0; row < 6; ++row) {
        for (int col = 0; col < 6; ++col) {
            reordered(row, col) = static_cast<double>(
                sophusInformation(
                    kG2oToSophus[row], kG2oToSophus[col]));
        }
    }
    return 0.5 * (reordered + reordered.transpose());
}

Eigen::Matrix<float, 6, 6> Optimizer::PoseInformationFromG2O(
    const Eigen::Matrix<double, 6, 6>& g2oInformation)
{
    static constexpr int kSophusToG2o[6] = {3, 4, 5, 0, 1, 2};
    Eigen::Matrix<float, 6, 6> reordered;
    for (int row = 0; row < 6; ++row) {
        for (int col = 0; col < 6; ++col) {
            reordered(row, col) = static_cast<float>(
                g2oInformation(
                    kSophusToG2o[row], kSophusToG2o[col]));
        }
    }
    return 0.5f * (reordered + reordered.transpose());
}

int Optimizer::PoseOptimization(
    Frame *pFrame, const Sophus::SE3f* posePrior,
    const Eigen::Matrix<float, 6, 6>* posePriorInformation,
    Eigen::Matrix<float, 6, 6>* visualInformation)
{
    if (visualInformation)
        visualInformation->setZero();
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    if (posePrior != nullptr && posePriorInformation != nullptr &&
        posePrior->matrix().allFinite() &&
        posePriorInformation->allFinite()) {
        g2o::VertexSE3Expmap* vPrior = new g2o::VertexSE3Expmap();
        vPrior->setEstimate(g2o::SE3Quat(
            posePrior->unit_quaternion().cast<double>(),
            posePrior->translation().cast<double>()));
        vPrior->setId(1);
        vPrior->setFixed(true);
        optimizer.addVertex(vPrior);

        g2o::EdgeSE3* priorEdge = new g2o::EdgeSE3();
        priorEdge->setVertex(
        ...
        priorEdge->setVertex(
            1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(
                   optimizer.vertex(1)));
        priorEdge->setMeasurement(g2o::SE3Quat());
        priorEdge->setInformation(
            PosePriorInformationForG2O(*posePriorInformation));
        optimizer.addEdge(priorEdge);
    }
    ...
    if (visualInformation) {
        Eigen::Matrix<double, 6, 6> g2oInformation =
            Eigen::Matrix<double, 6, 6>::Zero();
        const auto accumulateUnaryEdge = [&g2oInformation](auto* edge) {
            if (!edge || edge->level() != 0)
                return;
            edge->computeError();
            edge->linearizeOplus();
            const auto& jacobian = edge->jacobianOplusXi();
            g2oInformation.noalias() +=
                jacobian.transpose() * edge->information() * jacobian;
        };
        for (auto* edge : vpEdgesMono)
            accumulateUnaryEdge(edge);
        for (auto* edge : vpEdgesMono_FHR)
            accumulateUnaryEdge(edge);
        for (auto* edge : vpEdgesStereo)
            accumulateUnaryEdge(edge);
        *visualInformation = PoseInformationFromG2O(g2oInformation);
    }
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, MappingOperation& opr)
{
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        sort(
            orderedObservations.begin(), orderedObservations.end(),
