    ...
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
            ...
            if(mpRefKF==pKF)
            {
                ...
                {
                    KeyFrame* candidate = observation.first;
                    if(candidate &&
                       (!mpRefKF || candidate->mnId < mpRefKF->mnId))
                    {
                        mpRefKF = candidate;
                    }
                }
...
void MapPoint::SetDescriptorForReplay(const cv::Mat& descriptor)
{
    unique_lock<mutex> lock(mMutexFeatures);
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    if(!pRefKF || observations.count(pRefKF) == 0)
        pRefKF = orderedObservations.front().first;
