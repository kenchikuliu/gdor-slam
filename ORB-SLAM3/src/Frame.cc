...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &imRGB, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib, const cv::Mat &staticMask)
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    // Store static mask if provided
    if (!staticMask.empty()) {
        mStaticMask = staticMask.clone();
    }
    ...
    // ORB extraction with mask
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    ExtractORB(0,imGray,0,0,mStaticMask);
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1, const cv::Mat &staticMask)
{
    vector<int> vLapping = {x0,x1};
    if(flag==0)
        monoLeft = (*mpORBextractorLeft)(im,staticMask,mvKeys,mDescriptors,vLapping);
    else
        monoRight = (*mpORBextractorRight)(im,staticMask,mvKeysRight,mDescriptorsRight,vLapping);
}
