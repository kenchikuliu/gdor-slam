...
class ProcessingFlagGuard {
public:
    explicit ProcessingFlagGuard(std::atomic<bool>& flag) : flag_(flag)
    {
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        {
            ProcessingFlagGuard processing(mbProcessingKeyFrame);
            if(mpLastCurrentKF)
            {
...
int LoopClosing::KeyframesInQueue()
{
    unique_lock<mutex> lock(mMutexLoopQueue);
    return static_cast<int>(mlpLoopKeyFrameQueue.size());
}

bool LoopClosing::IsIdle()
{
    return !mbProcessingKeyFrame.load(std::memory_order_acquire) &&
        KeyframesInQueue() == 0 &&
        !isRunningGBA();
}
