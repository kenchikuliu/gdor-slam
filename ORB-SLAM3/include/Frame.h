...
public:
    ...
    // staticMask is optional: 1=static (extract features), 0=dynamic (skip features)
    void ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1, const cv::Mat &staticMask = cv::Mat());
...
public:
    ...
    // Static mask for dynamic object filtering (1=static, 0=dynamic)
    cv::Mat mStaticMask;

    // DyGeoFusion-SLAM+: Soft static weight for 3DGS optimization
    // CV_32FC1, values in [0, 1], where 1=static, 0=dynamic
    // mStaticWeight = 1 - P_dyn (dynamic probability)
    cv::Mat mStaticWeight;
