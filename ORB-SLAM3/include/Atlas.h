...
inline bool mapPointPassesGaussianStaticMask(MapPoint* pMP)
{
    if (pMP == nullptr)
        return false;

    bool has_masked_observation = false;
    for (const auto& observation : pMP->GetObservations()) {
        KeyFrame* pKF = observation.first;
        if (pKF == nullptr || pKF->imgStaticWeight.empty() ||
            pKF->imgStaticWeight.type() != CV_32FC1) {
            continue;
        }
        const int index = std::get<0>(observation.second);
        if (index < 0 ||
            index >= static_cast<int>(pKF->mvKeys.size())) {
            continue;
        }
        has_masked_observation = true;
        const cv::Point2f pixel = pKF->mvKeys[index].pt;
        const int x = cvRound(pixel.x);
        const int y = cvRound(pixel.y);
        if (x >= 0 && x < pKF->imgStaticWeight.cols &&
            y >= 0 && y < pKF->imgStaticWeight.rows &&
            pKF->imgStaticWeight.at<float>(y, x) >= 0.5f) {
            return true;
        }
    }
    return !has_masked_observation;
}
...
private:
    MappingOperation(
        ...
        : meOperationType(opr.meOperationType),
          ...
          mnMapPointsConsidered(opr.mnMapPointsConsidered),
          mnMapPointsRejectedStaticMask(
              opr.mnMapPointsRejectedStaticMask),
...
public:
    MappingOperation(
        ...
        : meOperationType(type),
          mfScale(scale),
          mT(T),
          mnMapPointsConsidered(0),
          mnMapPointsRejectedStaticMask(0)
    {
...
public:
    ...
    void addMapPoint(MapPoint* pMP)
    {
        ...
        ++mnMapPointsConsidered;
        if (!mapPointPassesGaussianStaticMask(pMP)) {
            ++mnMapPointsRejectedStaticMask;
            return;
        }
    ...
    std::size_t mapPointsConsidered() const
    {
        std::unique_lock<std::mutex> lock(mMutexMapPoints);
        return mnMapPointsConsidered;
    }

    std::size_t mapPointsRejectedStaticMask() const
    {
        std::unique_lock<std::mutex> lock(mMutexMapPoints);
        return mnMapPointsRejectedStaticMask;
    }
...
protected:
    ...
    std::size_t mnMapPointsConsidered;
    std::size_t mnMapPointsRejectedStaticMask;
    ...
    std::vector<std::tuple<
        ...
        cv::Mat/*staticWeight (DyGeoFusion-SLAM+)*/>> mvAssociatedKeyFrames;
