...
public:
    ...
    int static PoseOptimization(
        Frame* pFrame,
        const Sophus::SE3f* posePrior = nullptr,
        const Eigen::Matrix<float, 6, 6>* posePriorInformation = nullptr,
        Eigen::Matrix<float, 6, 6>* visualInformation = nullptr);
    static Eigen::Matrix<double, 6, 6> PosePriorInformationForG2O(
        const Eigen::Matrix<float, 6, 6>& sophusInformation);
    static Eigen::Matrix<float, 6, 6> PoseInformationFromG2O(
